...
## (Hybrid + Image Tuning + Vector/CAD + Mermaid + Vision Peeks/Looks + Image/Video exist + Write/Prose lanes)

## Status
- [ ] Single Open WebUI (OWUI) instance configured
...
- [ ] Vision split into two endpoints configured:
...
- [ ] chat-image and chat-video endpoints deployed and reachable
- [ ] MVP generation remains manual via OWUI's ComfyUI integration
- [ ] Writing split into two endpoints configured:
  - [ ] chat-write (creative/fiction/exotic mixes; 15B–35B-ish; may be Minis or Sparks)
  - [ ] chat-prose (giant/premium longform lane when allocated; typically Sparks; not always-on)
...
#### Writing lanes
- [ ] chat-write : creative/fiction/exotic writing specialist (may be down)
- [ ] chat-prose : giant premium prose lane (may be down)

#### Generation + other modalities
- [ ] chat-image : raster image generation endpoint (EXISTS); MVP still uses OWUI ComfyUI UI/manual trigger
- [ ] chat-video : video generation endpoint (EXISTS); MVP still uses OWUI UI/manual flow
...
- [ ] Images/video gen: produce "ready-to-run" prompts + defaults; user triggers generation via OWUI UI
- [ ] Audio/video read + TTS: route + instructions now; automate follow-up results later

#### Hybrid turns support
...
### Non-goals (for now)
- [ ] True parallel streaming text while image/video generation runs (NOT implementing)
- [ ] Middleware that mutates payloads (avoid LiteLLM in the mandatory request path) (NOT implementing)
- [ ] AutoRoute directly invoking chat-image/chat-video endpoints (deferred; MVP uses OWUI integration UI) (NOT implementing)
- [ ] Fully automated browsing/citations; only a research preamble for now (NOT implementing)
...
- [ ] chat-looks: heavy VLM (often the same heavy allocation as chat-heavy when available)

#### Writing lanes
- [ ] chat-write: creative/exotic mixes (15B–35B-ish)
- [ ] chat-prose: giant premium longform (when allocated)

#### Media
- [ ] ComfyUI configured via OWUI integration (canonical MVP for raster generation)
- [ ] chat-image and chat-video endpoints are deployed and reachable, but not auto-invoked in MVP
...
#### Vision
- [ ] chat-peeks => CHAT_PEEKS_VIP
- [ ] chat-looks => CHAT_LOOKS_VIP

#### Writing
- [ ] chat-write => CHAT_WRITE_VIP
- [ ] chat-prose => CHAT_PROSE_VIP

#### Media (existence noted)
- [ ] chat-image => CHAT_IMAGE_VIP   (EXISTS; OWUI UI/manual flow in MVP)
- [ ] chat-video => CHAT_VIDEO_VIP   (EXISTS; OWUI UI/manual flow in MVP)
...
- [ ] AutoRoute may probe availability for chat-devel, chat-looks, chat-write, chat-prose to decide fallback quickly
...
- [ ] Implement JSON schema with fields:
  - [ ] "target": "chat-quick" | "chat-heavy" | "chat-devel" | "chat-peeks" | "chat-looks" | "chat-write" | "chat-prose" | "chat-image" | "chat-video" | "chat-trans" | "chat-watch" | "chat-speak"
...
#### Step 0: Ripcord detection
- [ ] If user issues override command: set/clear override, acknowledge briefly, continue routing
- [ ] Override does not supersede artifact-first rules or vision routing

#### Step 1: Artifact-first exceptions (text artifacts that look "image-y")

##### 1A) Vector/CAD/parametric outputs
- [ ] If user mentions: "vector", "SVG", "Inkscape", "Illustrator", "DXF" => artifact_mode="svg", needs_image=false
- [ ] If user mentions: "OpenSCAD", "3D print", "STL", "parametric", "CAD", "3D model" => artifact_mode="openscad", needs_image=false
- [ ] If user also asks "render/raster/image/preview", then allow needs_image=true in addition to artifact

##### 1B) Flow/logic diagrams
- [ ] If user mentions: "flowchart", "state machine", "sequence", "decision tree", "pipeline diagram" => artifact_mode="mermaid" (default) or "ascii" (fallback), needs_image=false
- [ ] Only set needs_image=true if user explicitly asks "as an image" / "generate a picture"

#### Step 2: Vision vs generation vs other modalities
...
- [ ] Escalate to chat-looks when:
  - [ ] User explicitly requests deep visual reasoning (multi-image comparison, complex chart interpretation), OR
  - [ ] User says "use the big vision model", OR
  - [ ] Peeks fails and user requests escalation
- [ ] Fallback:
...
##### 2B) Writing lanes
- [ ] Route to chat-write when:
  - [ ] User asks for fiction/creative writing, roleplay, "uncensored/exotic mix" vibes, dialog-heavy scenes
- [ ] Route to chat-prose when:
  - [ ] User asks for premium longform/polish and the giant lane is allocated ("best prose", "editorial pass", "long essay", "high consistency")
- [ ] Fallback:
  - [ ] If chat-prose is unavailable, fall back to chat-write (if available) else chat-heavy
  - [ ] If chat-write is unavailable, fall back to chat-heavy (or chat-quick for light edits)

##### 2C) Audio/video read + TTS
...
##### 2D) Raster image generation intent (ComfyUI path; chat-image exists but not auto-invoked in MVP)
- [ ] If user requests raster imagery ("draw", "sketch", "generate an image", "make a picture", "mockup", "wireframe") and artifact_mode="none":
  - [ ] needs_image=true
  - [ ] Primary target remains a text model (chat-quick or chat-heavy) for prompt planning in MVP
  - [ ] image_action:
    - [ ] make_now: produce a ready-to-run prompt + defaults
    - [ ] tune_first: do a short interview (2–5 questions), then produce prompt
  - [ ] Even though chat-image exists, AutoRoute does not auto-dispatch to it in MVP (safety/UX: OWUI Comfy integration is canonical)
  - [ ] Future phase: optionally allow AutoRoute to invoke chat-image directly and post results

##### 2E) Video generation intent (chat-video exists but not auto-invoked in MVP)
- [ ] chat-video exists; MVP treats it like chat-image:
  - [ ] generate ready-to-run prompt/workflow guidance
  - [ ] user triggers via OWUI/manual flow
  - [ ] direct invocation deferred
...
- [ ] If needs_image=true AND user also needs substantial reasoning/planning:
  ...
  - [ ] Assistant asks: "I'm going to answer this in both a message and an image. Which would you like first: (A) the written answer or (B) the image prompt?"
...
- [ ] When mode=research:
  - [ ] Prepend research preamble: "State assumptions. Verify dates. If unsure, say so. Cite sources if available. Don't guess."
...
- [ ] Writing lanes may override default text routing when explicitly requested
- [ ] Then override applies to remaining text-only routes (quick/heavy/devel)
...
### Raster image/video generation (MVP = manual OWUI flow)
- [ ] chat-image and chat-video endpoints exist and are reachable
- [ ] MVP behavior remains: AutoRoute produces ready-to-run prompts + defaults; user triggers generation via OWUI UI
- [ ] AutoRoute does not submit generation jobs directly in MVP

### Raster image prompts (ComfyUI)
- [ ] If needs_both=true: ask which first (A text / B image prompt)
- [ ] If image_action="make_now":
  - [ ] Provide a ready-to-run prompt verbatim + minimal defaults
...
### Writing outputs (chat-write/chat-prose)
- [ ] chat-write: prioritize creativity/dialog/scene generation; accept "exotic" tone requests
- [ ] chat-prose: prioritize coherence, polish, longform structure, editorial refinement
...
- [ ] chosen VIP/class
- [ ] latency
- [ ] availability/fallback events for chat-devel, chat-looks, chat-write, chat-prose
- [ ] errors/retries
...
- [ ] Implement router call (truncated input) + strict JSON parse (or rules-only MVP)
...
### Phase E (later) — Automation and follow-ups
- [ ] Optional: AutoRoute invokes chat-image/chat-video directly and posts results as follow-up messages
- [ ] Optional: AutoRoute invokes ASR/TTS and posts artifacts
- [ ] Optional: auto-start/stop specialist lanes (devel/prose) based on demand
